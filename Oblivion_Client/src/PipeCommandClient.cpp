...
namespace OblivionEye {
    ...
    static void SendUtf8(const std::wstring& type, const std::wstring& w) { if (!PipeClient::Instance().IsRunning()) return; int len = WideCharToMultiByte(CP_UTF8,0,w.c_str(),-1,nullptr,0,nullptr,nullptr); std::string utf8(len,'\0'); WideCharToMultiByte(CP_UTF8,0,w.c_str(),-1,&utf8[0],len,nullptr,nullptr); if(!utf8.empty()&&utf8.back()=='\0') utf8.pop_back(); std::string t(type.begin(), type.end()); PipeClient::Instance().Send("INFO|"+t+"|"+utf8); }

    void PipeCommandClient::HandleCommandLine(const std::string& line) {
        static std::deque<DWORD> recentTicks; static const size_t MAX_RECENT=20; static DWORD windowMs=3000; DWORD now = GetTickCount();
        while(!recentTicks.empty() && now - recentTicks.front() > windowMs) recentTicks.pop_front(); if (recentTicks.size() >= MAX_RECENT) { return; } recentTicks.push_back(now);
        ...
        const DWORD RISK_COOLDOWN_MS = 2000; // 2s minimal interval
        const unsigned ABUSE_THRESHOLD = 5;  // after 5 rapid violations we ignore silently
        ...
        if (cmd == "CLOSE_CLIENT") { ExitProcess(0); }
            ...
            std::string rest; getline(iss,rest); if(!rest.empty()&&rest[0]==' ')rest.erase(0,1); if(rest.empty()) return; auto w = Utf8ToW(rest); DWORD remain = cooldownRemain(lastQuarantineTick, now, RISK_COOLDOWN_MS);
            if (remain) {
        ...
        else if (cmd == "SET_INTERVAL") { std::string det; unsigned ms=0; iss>>det>>ms; if(!det.empty()&&ms>0) { bool changed = DetectorScheduler::Instance().SetIntervalOverride(Utf8ToW(det), ms); SendUtf8(L"RESULT", changed ? (L"SET_INTERVAL OK " + Utf8ToW(det) + L"=" + std::to_wstring(ms)) : (L"SET_INTERVAL FAIL " + Utf8ToW(det))); } }
        else if (cmd == "CLEAR_INTERVAL") { std::string det; iss>>det; if(!det.empty()) { bool ok = DetectorScheduler::Instance().ClearIntervalOverride(Utf8ToW(det)); SendUtf8(L"RESULT", ok? (L"CLEAR_INTERVAL OK "+Utf8ToW(det)) : (L"CLEAR_INTERVAL FAIL "+Utf8ToW(det))); } }
