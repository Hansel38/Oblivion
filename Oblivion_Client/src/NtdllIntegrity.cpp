...
bool NtdllIntegrity::CaptureSubsectionHashes() {
    ...
    for(size_t i=0;i<chunks;++i){ size_t off=i*chunk; size_t len=(off+chunk<=size)?chunk:(size-off); m_chunkHashes[i]=Fnv1a64(base+off,len);} return true;
}
...
bool NtdllIntegrity::MapFreshDiskText(std::vector<unsigned long long> &diskChunks, unsigned long long &diskHash) const {
    ...
    diskHash=Fnv1a64(textBase,textSize);
    const size_t chunk=OblivionEye::Config::CHUNK_SIZE; size_t chunks=(textSize+chunk-1)/chunk; diskChunks.resize(chunks);
    for(size_t i=0;i<chunks;++i){ size_t off=i*chunk; size_t len=(off+chunk<=textSize)?chunk:(textSize-off); diskChunks[i]=Fnv1a64(textBase+off,len); }
...
void NtdllIntegrity::SaveBaseline() {
    ...
    f<<m_baselineHash<<L" "<<m_chunkHashes.size(); for(auto h: m_chunkHashes) f<<L" "<<h;
    f<<L" "<<m_diskHash<<L" "<<m_diskChunkHashes.size(); for(auto h: m_diskChunkHashes) f<<L" "<<h;
    f<<L" "<<HmacToHex()<<L"\n";
}

bool NtdllIntegrity::LoadBaseline() {
    std::wifstream f(L"ntdll_baseline.txt"); if(!f) return false; unsigned long long chunkCount=0,diskChunkCount=0; std::wstring hhex;
    if(!(f>>m_baselineHash>>chunkCount)) return false; m_chunkHashes.resize(static_cast<size_t>(chunkCount)); for(size_t i=0;i<m_chunkHashes.size();++i) f>>m_chunkHashes[i];
    ...
    if(!(f>>hhex)) return false; if(!HexToHmac(hhex)) return false; m_hmacValid=true; if(!VerifyHmac()){ Log(L"NtdllIntegrity baseline HMAC mismatch, ignoring stored baseline"); m_chunkHashes.clear(); m_diskChunkHashes.clear(); m_hmacValid=false; return false; }
...
void NtdllIntegrity::CaptureBaseline() {
    ...
    unsigned char* base=nullptr; size_t size=0; if(!GetNtdllTextRegion(base,size)) return; m_baselineHash=Fnv1a64(base,size); CaptureSubsectionHashes();
...
bool NtdllIntegrity::Check() {
    unsigned char* base=nullptr; size_t size=0; if(!GetNtdllTextRegion(base,size)) return false; auto current=Fnv1a64(base,size);
    if(m_baselineCaptured && current!=m_baselineHash){ const size_t chunk=OblivionEye::Config::CHUNK_SIZE; size_t chunks=(size+chunk-1)/chunk; std::wstring deltaInfo;
        for(size_t i=0;i<chunks && i<m_chunkHashes.size(); ++i){ size_t off=i*chunk; size_t len=(off+chunk<=size)?chunk:(size-off); unsigned long long h=Fnv1a64(base+off,len);
            if(h!=m_chunkHashes[i]){ if(IntegrityChunkWhitelist::IsWhitelisted(L"ntdll.dll", i)) continue; deltaInfo+=L"[m"+std::to_wstring(i)+L"@0x"+std::to_wstring(static_cast<unsigned long long>(off))+L"]"; if(m_diskCaptured && i<m_diskChunkHashes.size()){ if(m_diskChunkHashes[i]==h) deltaInfo+=L"(=disk)"; else deltaInfo+=L"(!disk)"; } }
        }
        if(deltaInfo.empty()) return false; EventReporter::SendDetection(L"NtdllIntegrity", L"ntdll .text modified chunks:"+deltaInfo); ShowDetectionAndExit(L"ntdll integrity mismatch "+deltaInfo); return true; }
    return false;
}
