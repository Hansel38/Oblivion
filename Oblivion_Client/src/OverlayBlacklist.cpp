...
namespace OblivionEye {
    ...
    void AddBlacklistedWindowTitle(const std::wstring& titleSubstr) {
        ...
        if (std::find(g_titles.begin(), g_titles.end(), low) == g_titles.end()) g_titles.push_back(low);
    }
    void AddBlacklistedWindowClass(const std::wstring& classSubstr) {
        auto low = ToLower(classSubstr);
        if (std::find(g_classes.begin(), g_classes.end(), low) == g_classes.end()) g_classes.push_back(low);
    }
