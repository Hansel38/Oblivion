...
void Gdi32Integrity::CaptureBaseline() {
    if (m_baselineCaptured)
        return;
    unsigned char *base = nullptr; size_t size = 0;
    if (!GetTextSection(L"gdi32.dll", base, size)) return;
    m_baselineHash = Fnv1a64(base, size);
    CaptureSubsectionHashes();
    m_baselineCaptured = true;
    Log(L"Gdi32Integrity baseline captured");
}
...
bool Gdi32Integrity::Check() {
    ...
    for (size_t i = 0; i < n && i < m_chunkHashes.size(); ++i) {
        ...
        if (h != m_chunkHashes[i]) {
            if (IntegrityChunkWhitelist::IsWhitelisted(L"gdi32.dll", i))
                continue;
