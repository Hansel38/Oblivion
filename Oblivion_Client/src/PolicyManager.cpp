...
namespace OblivionEye {
    ...
    static std::string WToUtf8(const std::wstring& w) { int len = WideCharToMultiByte(CP_UTF8,0,w.c_str(),-1,nullptr,0,nullptr,nullptr); std::string s(len,'\0'); WideCharToMultiByte(CP_UTF8,0,w.c_str(),-1,&s[0],len,nullptr,nullptr); if(!s.empty()&&s.back()=='\0') s.pop_back(); return s; }

    static void ParsePolicyContent(const std::string& data) {
        ...
        for (auto& raw : lines) {
            ...
            if (line[0]=='[') { if(line=="[process]")sec=PROC; else if(line=="[module]")sec=MOD; else if(line=="[driver]")sec=DRV; else if(line=="[overlay_title]")sec=OVT; else if(line=="[overlay_class]")sec=OVC; else if(line=="[publisher]")sec=PUB; else if(line=="[prolog]")sec=PROLOG; else if(line=="[chunk_whitelist]")sec=CHUNKWL; else if(line=="[interval]")sec=INTERVAL; else sec=NONE; continue; }
            ...
            case PROLOG: { std::wstringstream ws(wline); std::wstring mod, fn; unsigned bytes=8; ws>>mod>>fn>>bytes; if(!mod.empty()&&!fn.empty()){ std::string narrow = WToUtf8(fn); PrologHookChecker::Instance().AddTarget(mod,narrow,bytes);} } break; 
            case CHUNKWL: { std::wstringstream ws(wline); std::wstring mod; size_t idx; ws>>mod>>idx; if(!mod.empty()) IntegrityChunkWhitelist::Add(mod, idx); } break; 
            case INTERVAL: { std::wstringstream ws(wline); std::wstring name; unsigned val=0; ws>>name>>val; if(!name.empty() && val>0) { DetectorScheduler::Instance().SetIntervalOverride(name, val); } } break;
            default: break; }
        }
    ...
    bool PolicyManager::LoadPolicy(const std::wstring& path) {
        std::ifstream f(path, std::ios::binary); if(!f) { Log(L"Policy load failed, using embedded fallback"); ParsePolicyContent(kEmbeddedPolicy); return false; }
        std::string data((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>()); ParsePolicyContent(data); Log(L"Policy loaded from "+path); return true;
    }
    ...
    bool PolicyManager::SavePolicy(const std::wstring& path) {
        ...
        // intervals
        auto overrides = DetectorScheduler::Instance().GetIntervalOverrides(); if(!overrides.empty()){ writeSection("[interval]"); for(auto& kv: overrides){ f<<WToUtf8(kv.first)<<' '<<kv.second<<"\n"; } }
        f.flush(); return true;
    }
}
