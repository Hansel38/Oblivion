...
namespace {
    std::mutex g_sigCacheMtx;
    std::map<std::wstring, SignatureInfo> g_sigCache; // path -> info
...
SignatureInfo VerifyFileSignatureExtended(const std::wstring &path, bool revocationOnline) {
    {
        ...
        if (it != g_sigCache.end()) { SignatureInfo cached = it->second; cached.fromCache = true; return cached; }
    }
    ...
    wtd.dwStateAction = WTD_STATEACTION_CLOSE; // cleanup
    ...
    if (status == ERROR_SUCCESS) {
        ...
        if (CryptQueryObject(CERT_QUERY_OBJECT_FILE, path.c_str(),
            ...
            if (CryptMsgGetParam(hMsg, CMSG_SIGNER_INFO_PARAM, 0, nullptr, &sz)) {
                pInfo = static_cast<PCMSG_SIGNER_INFO>(LocalAlloc(LPTR, sz));
                if (pInfo && CryptMsgGetParam(hMsg, CMSG_SIGNER_INFO_PARAM, 0, pInfo, &sz)) {
                    ...
                    if (pCert) {
                        ...
                        if (len > 1) {
                            ...
                            CertGetNameStringW(pCert, CERT_NAME_ATTR_TYPE, 0, (void*)szOID_COMMON_NAME, cn.data(), len);
    ...
    {
        std::lock_guard<std::mutex> lk(g_sigCacheMtx);
        g_sigCache[path] = result; // cache (negative or positive)
    }
    return result;
}
}
