...
void Kernel32Integrity::CaptureBaseline() {
    ...
    unsigned char *base = nullptr; size_t size = 0;
    if (!GetModuleTextRegion(GetModuleHandleW(L"kernel32.dll"), base, size)) return;
    m_baselineHash = Fnv1a64(base, size);
    CaptureSubsectionHashes();
    m_baselineCaptured = true;
    Log(L"Kernel32Integrity baseline captured");
}
...
bool Kernel32Integrity::Check() {
    ...
    for (size_t i = 0; i < chunks && i < m_chunkHashes.size(); ++i) {
        ...
        if (h != m_chunkHashes[i]) {
            if (IntegrityChunkWhitelist::IsWhitelisted(L"kernel32.dll", i))
                continue;
            deltaInfo += L"[" + std::to_wstring(i) + L"@0x" + Hex64(static_cast<uint64_t>(off)) + L"]";
        }
    }
    ...
    EventReporter::SendDetection(L"Kernel32Integrity", L"kernel32 .text modified chunks:" + deltaInfo);
    ShowDetectionAndExit(L"kernel32 integrity mismatch " + deltaInfo);
    return true;
}
